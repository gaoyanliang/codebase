 ...
 *
 * <p>省份 是一组直接或间接相连的城市，组内不含其他没有相连的城市。
 *
 * <p>给你一个 n x n 的矩阵 isConnected ，其中 isConnected[i][j] = 1 表示第 i 个城市和第 j 个城市直接相连，而 isConnected[i][j]
 * = 0 表示二者不直接相连。
 *
 * <p>返回矩阵中 省份 的数量。 提示：
 *
 * <p>1 <= n <= 200 n == isConnected.length n == isConnected[i].length isConnected[i][j] 为 1 或 0
