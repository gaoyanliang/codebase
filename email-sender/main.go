...
func main() {
	...
	var location string
	location = os.Getenv("MAIL_LOCATION")
	nyc, _ := time.LoadLocation(location)
...
func getParts() map[string]interface{} {
	wrapMap := map[string]func() interface{}{
		"one":       func() interface{} { return common.GetONE() },
		"english":   func() interface{} { return common.GetEnglish() },
		"poem":      func() interface{} { return common.GetPoem() },
		"wallpaper": func() interface{} { return common.GetWallpaper() },
	}
...
func batchSendMail() {
	...
	if isDev() {
		weather := common.GetWeather(users[0].Local)
	...
	for _, user := range users {
		...
		go func(user User) {
			defer wg.Done()
			weather := common.GetWeather(user.Local)
			lock.Lock()
			parts["weather"] = weather
			html := generateHTML(HTML, parts)
			lock.Unlock()
